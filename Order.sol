...
contract Order{
  ...
  uint private deliverersCount;
	...
	constructor () public { // constructor, creates order. we map starting from id=1
        ...
        addTrace(2,"some coordinates", "name or address of actual owner","timestamp");
        triggered=false;
        delivery=false;
        received=false;
	}
    ...
    // returns global number of retailers
    // smart to smart comm
    function updateNumberOfRetailers (address addr) public view returns (uint){
        
        Retailers p = Retailers(addr);     
        retailersCount = p.getNumberOfRetailers();
        return p.getNumberOfRetailers();
       
    }
    
    // returns global number of vendors
    // smart to smart comm
    function updateNumberOfVendors (address addr) public view returns (uint){
        
        Vendors s = Vendors(addr);
        vendorsCount = s.getNumberOfVendors();
        return s.getNumberOfVendors();
        
    }
    
       // returns global number of deliverers
    // smart to smart comm
    function updateNumberOfDeliverers (address addr) public view returns (uint){
        
        Deliverers s = Deliverers(addr);
        deliverersCount = s.getNumberOfDeliverers();
        return s.getNumberOfDeliverers();
