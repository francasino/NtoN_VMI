...
contract Retailers{
    ...
    mapping(uint => Retailer) private retailerChanges; //

    uint private retailerCount;
    ...
    address constant public retailer = 0xE0f5206BBD039e7b0592d8918820024e2a7437b9; // who registers the token into system. 
    address constant public retailer2 = 0xE0F5206bbd039e7b0592d8918820024E2A743222;

    constructor () public { // constructor, inserts new token in system. we map starting from id=1, hardcoded values of all
        addRetailer("Retailer","1130009112019","Local seller of vegetables"); //
        
    }
    
    // add retailer to the list
    function addRetailer(string memory _name, string memory _timestamp, string memory _description) public {
        
        retailerCount++;
        retailerChanges[retailerCount].id = retailerCount;
        retailerChanges[retailerCount].name = _name; 
        retailerChanges[retailerCount].timestamp = _timestamp; 
        retailerChanges[retailerCount].description = _description; 
        retailerChanges[retailerCount].active = true; 
        retailerChanges[retailerCount].maker = msg.sender;
        emit updateEvent(); // trigger event 
    }

    function changeStatus (uint memory _id, bool memory _active) public {
        require(_id > 0 && _id <= retailerCount); 
        retailerChanges[retailerCount].active = _active;
        emit changeStatusEvent(); // trigger event 
    }

    function getRetailer (uint memory _id) public view returns (Retailer memory)  {
        require(_id > 0 && _id <= retailerCount);  
        require(msg.sender == retailerChanges[_id].maker); // only if he is the author of the content
        
        return retailerChanges[_id];
    }
    
    // returns global number of status, needed to iterate the mapping and to know info.
    function getNumberOfRetailers () public view returns (uint){    
        //tx.origin
